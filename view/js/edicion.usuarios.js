...
$(document).ready(function(){
    ...
    $("#agregarContacto").click(function(){
        ...
        if(nombre=='' || cargo==''|| direccion==''|| celular==''|| correo=='' || coddepartamento=="000" || codprovincia =="000" || coddistrito == "000"){
...
function listar() {
    $.post
            (
                    "../controller/gestionarCliente.listar.controller.php"
...
function cargarcontactos() {
    $.post
    (
            "../controller/gestionarCliente.listar.contacto.controller.php"
...
if (datosJSON.estado === 200) {
    ...
    for(i = 0; i < resultado.datos.length; i++){
     ...
     var adicion = '<tr><th scope="row">-</th>' + 
     '<td class="nomcontacto">' + resultado.datos[i].nom_contacto + '</td><td class="cargocontacto">' + resultado.datos[i].cargo_contacto +'</td>' + 
     '<td class="direccion">' + resultado.datos[i].direccion + '</td><td class="celular">' + resultado.datos[i].celular +'</td>' + 
     '<td class="cargo">' + resultado.datos[i].correo + '</td>' + 
     '<td><div class="widget-content-right widget-content-actions">' + 
     '<button type="button" name="delete" class="border-0 btn-transition btn btn-outline-danger delete"><i class="fa fa-trash-alt"></i></button></div></td></tr>';
...
function cargarRutasUsu() {
    $.post
    (
            "../controller/gestionarCliente.listar.departamento.controller.php"
...
if (datosJSON.estado === 200) {
    ...
    for(i = 0; i < resultado.datos.length; i++){
        ...
        var o = new Option(resultado.datos[i].departamento, resultado.datos[i].coddepartamento);
...
//swal("Error", datosJSON.mensaje , "error"); 
    ...
    $('#cbodepartamentousu').change(function() {
        ...
        $.post
        (
                "../controller/gestionarCliente.listar.provincia.controller.php",
                {
    ...
    if (datosJSON.estado === 200) {
        ...
        for(i = 0; i < resultado.datos.length; i++){
            ...
            var o = new Option(resultado.datos[i].provincia, resultado.datos[i].codprovincia);
    ...
    $('#cboprovinciausu').change(function() {
        ...
        $.post
        (
                "../controller/gestionarCliente.listar.distrito.controller.php",
                {
                    p_coddepartamento: $("#cbodepartamentousu option:selected").val(),
    ...
    if (datosJSON.estado === 200) {
        ...
        for(i = 0; i < resultado.datos.length; i++){
            ...
            var o = new Option(resultado.datos[i].distrito, resultado.datos[i].coddistrito);
